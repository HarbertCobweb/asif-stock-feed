...
const HOLDINGS = [
  ...
  { symbol: "PHYS", name: "Sprott Physical Gold Trust", sector: "Precious Metals Fund" },
  { symbol: "PSLV", name: "Sprott Physical Silver Trust", sector: "Precious Metals Fund" },
  { symbol: "WM", name: "Waste Management", sector: "Industrials" },
  { symbol: "UBER", name: "Uber Technologies", sector: "Industrials" },
  { symbol: "SGDJ", name: "Sprott Junior Gold Miners ETF", sector: "Precious Metals Fund" },
...
function createEmptyStock(holding, errorMessage = null) {
  return {
...
async function fetchAlphaVantageDaily(holding) {
  ...
  url.searchParams.set("symbol", holding.symbol);
  ...
  if (!response.ok) {
    throw new Error(
      `Alpha Vantage request failed for ${holding.symbol}: ${response.status} ${response.statusText}`
    );
  }
  ...
  try {
    ...
    throw new Error(`Alpha Vantage returned invalid JSON for ${holding.symbol}.`);
  }

  if (payload["Error Message"]) {
    throw new Error(`Alpha Vantage symbol error for ${holding.symbol}: ${payload["Error Message"]}`);
  }

  if (payload.Note) {
    throw new Error(`Alpha Vantage rate limit reached for ${holding.symbol}: ${payload.Note}`);
  }

  if (payload.Information) {
    throw new Error(`Alpha Vantage response for ${holding.symbol}: ${payload.Information}`);
  }

  const series = payload["Time Series (Daily)"];

  if (!series || typeof series !== "object") {
    throw new Error(`No daily time series returned for ${holding.symbol}.`);
  }

  const dates = Object.keys(series).sort((a, b) => b.localeCompare(a));

  if (dates.length < 2) {
    throw new Error(`Not enough daily records returned for ${holding.symbol}.`);
  }
  ...
  const price = normalizeNumber(latest["4. close"]);
  const open = normalizeNumber(latest["1. open"]);
  const volume = normalizeNumber(latest["5. volume"]);
  const previousClose = normalizeNumber(previous["4. close"]);

  if (price === null || previousClose === null) {
    throw new Error(`Incomplete daily quote data returned for ${holding.symbol}.`);
  }
...
async function fetchStockData(holdings) {
  if (!ALPHAVANTAGE_API_KEY) {
    throw new Error("ALPHAVANTAGE_API_KEY is missing from the environment variables.");
  }
  ...
  for (const holding of holdings) {
    try {
      const quote = await fetchAlphaVantageDaily(holding);
      results.push(quote);
      console.log(`Loaded ${holding.symbol}`);
    } catch (error) {
      console.error(error.message);
      errors.push({ symbol: holding.symbol, error: error.message });
      results.push(createEmptyStock(holding, error.message));
    }
  }

  const successfulResults = results.filter(stock => stock.price !== null);

  if (successfulResults.length === 0) {
    throw new Error(
      `All Alpha Vantage requests failed. First error: ${errors[0]?.error || "Unknown Alpha Vantage error"}`
    );
  }
...
async function createAndSaveStockPayload(holdings = HOLDINGS) {
  const stockData = await fetchStockData(holdings);

  const payload = {
    updatedAt: new Date().toISOString(),
    source: "Alpha Vantage — Daily Time Series",
    count: stockData.length,
    successfulCount: stockData.filter(stock => stock.price !== null).length,
...
app.get("/api/refresh", async (req, res) => {
  try {
    const payload = await createAndSaveStockPayload();
...
function sendXmlFeed(req, res) {
  ...
  res.type("application/xml").send(stockDataToXml(payload));
}

app.get("/data/stocks.ixml", sendXmlFeed);
app.get("/data/stocks.xml", sendXmlFeed);
...
app.get("/api/status", (req, res) => {
  ...
  if (fs.existsSync(STOCKS_JSON_FILE)) {
    try {
      ...
      cache = { error: "Cache exists but could not be parsed." };
    }
  }
  ...
  res.json({
    ...
    apiKeyConfigured: Boolean(ALPHAVANTAGE_API_KEY),
...
app.listen(PORT, () => {
  ...
  console.log(`Alpha Vantage API key configured: ${Boolean(ALPHAVANTAGE_API_KEY)}`);
});
